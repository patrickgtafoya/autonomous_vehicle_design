 ...
 */
void MyMPU6050::meanSensor(struct SensorData* mv, struct SensorData* sd, struct SensorData* ov){
...
void MyMPU6050::setOffsetValues(struct SensorData mv, struct SensorData* ov){
  ov->accelX -= mv.accelX / 1.5;
  ov->accelY -= mv.accelY / 1.5;
  ov->accelZ -= (mv.accelZ) / 1.5;
  ov->gyroX -= mv.gyroX / 2.5;
  ov->gyroY -= mv.gyroY / 1.5;
  ov->gyroZ -= mv.gyroZ / 1.5;
}
/*
 *  clearOffsetValues()
 *      return: void
...
/*
 *  Functions to retur raw data with offset calibration
 */
...
double MyMPU6050::calcAccelPitch(SensorData accelData)
{
  return atan(-accelData.accelX / sqrt(pow(accelData.accelY,2) + pow(accelData.accelZ,2))) * 180.0/PI;
}

double MyMPU6050::calcAccelRoll(SensorData accelData)
{
  return atan2(accelData.accelY, accelData.accelZ) * 180.0/PI;
}

void MyMPU6050::integrateGyro(AngleData* gyroDelta, SensorData gyroData, unsigned long tInterval)
{
  gyroDelta->x = (double)gyroData.gyroX / 131 * tInterval / 1000.0;
  gyroDelta->y = (double)gyroData.gyroY / 131 * tInterval / 1000.0;
  gyroDelta->z = (double)gyroData.gyroZ / 131 * tInterval / 1000.0;
}

double MyMPU6050::complimentaryFilter(double gyroAngle, double accelAngle, double alpha)
{
 return alpha*gyroAngle + (1 - alpha)*accelAngle;
}

void MyMPU6050::filterData(AngleData* compPos, AngleData gyroDelta, double aPitch, double aRoll, double alpha)
{
  compPos->x = complimentaryFilter(gyroDelta.x, aRoll, alpha);
  compPos->y = complimentaryFilter(gyroDelta.y, aPitch, alpha);
  compPos->z = gyroDelta.z;
}

double MyMPU6050::lowPassRoll(double rollF, double roll)
{
  return 0.94*rollF + 0.06*roll;
}

double MyMPU6050::lowPassPitch(double pitchF, double pitch)
{
  return 0.94*pitchF + 0.06*pitch;
}

void MyMPU6050::calcGyroPosition(AngleData* gyro, AngleData delta)
{
  gyro->x += delta.x;
  gyro->y += delta.y;
  gyro->z += delta.z;
